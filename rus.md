...
Ввод-вывод в node асинхронен, поэтому взаимодействие с диском и сетью происходит через различные способы управления асинхронным кодом (обещания, генераторы, функции обратного вызова и т.п.). Следующий код отдает файл браузеру через функцию обратного вызова:
...
Третий аргумент, `next(err)`, является функцией обратного вызова, сообщающей отправителю что можно послать еще данные. Если вы вызовите ее с параметром `err`, в потоке будет создано событие `'error'`.
...
_У классических потоков на чтение для остановки и продолжения есть методы `.pause()` и `.resume()`, но их использования следует избегать. Если вам необходим этот фуникционал - рекомендуется не создавать логику самостоятельно, а использовать модуль [through](https://npmjs.org/package/through)._
